...
class UnitConverter {
  ...
  static const Map<String, double> _ambiguousConversionTable = {
    'tbsp': 3.0, // Convert tablespoons to teaspoons
    'tablespoons': 3.0,
    'tsp': 1.0, // Keep teaspoons as teaspoons
    'teaspoons': 1.0,
    'cups': 48.0, // Convert cups to teaspoons
  };
  ...
  List<Ingredient> combineIngredients(List<Ingredient> ingredients) {
    ...
    for (var entry in ambiguousIngredients.entries) {
      ...
      if (combinedQuantities.containsKey(key)) {
        ...
        // No definite unit entries, consolidate ambiguous units to teaspoons
        double totalQuantity = ambiguousList.fold(
            0,
            (sum, ingredient) =>
                sum + convertToBaseUnit(ingredient.quantity, ingredient.unit));
        combinedQuantities[key] = totalQuantity; // Consolidated to teaspoons
        finalUnits[key] = 'teaspoons'; // Choose the smallest unit (teaspoons)
      }
    }
