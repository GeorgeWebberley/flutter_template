...
class UnitConverter {
  ...
  // Ambiguous units conversion to base unit (smallest common unit - teaspoons)
  static const Map<String, double> _ambiguousConversionTable = {
    'tbsp': 3.0, // 1 tablespoon = 3 teaspoons
    'tablespoons': 3.0,
    'tsp': 1.0, // 1 teaspoon = 1 teaspoon (base unit)
    'teaspoons': 1.0,
    'cups': 48.0, // 1 cup = 48 teaspoons
  };

  // Units with their larger equivalents
  static const Map<String, String> _largerUnits = {
    'ml': 'liters',
    'milliliters': 'liters',
    'g': 'kilograms',
    'grams': 'kilograms'
  ...
  static double convertFromBaseUnit(double baseQuantity, String unit) {
    ...
    if (isVolumeUnit(normalizedUnit)) {
      ...
      return baseQuantity / _ambiguousConversionTable[normalizedUnit]!;
    }
    return baseQuantity; // Return the base quantity as-is for unrecognized units
  }

  /// Determine if a quantity can be cleanly converted to its larger unit.
  bool canConvertToLargerUnit(double quantity, String unit) {
    if (!_largerUnits.containsKey(unit)) return false;
    String largerUnit = _largerUnits[unit]!;
    double conversionFactor = _volumeConversionTable.containsKey(unit)
        ? _volumeConversionTable[largerUnit]! / _volumeConversionTable[unit]!
        : _weightConversionTable[largerUnit]! / _weightConversionTable[unit]!;
    double convertedQuantity = quantity / conversionFactor;
    return convertedQuantity >= 1.0 &&
        convertedQuantity == convertedQuantity.roundToDouble();
  }
  ...
  List<Ingredient> combineIngredients(List<Ingredient> ingredients) {
    ...
    for (var entry in ambiguousIngredients.entries) {
      ...
      if (combinedQuantities.containsKey(key)) {
        ...
        // No definite unit entries, consolidate ambiguous units to the largest unit available
        String largestUnit = ambiguousList.map((ing) => ing.unit).reduce(
            (a, b) => _ambiguousConversionTable[normalizeUnit(a)]! >
                    _ambiguousConversionTable[normalizeUnit(b)]!
                ? a
                : b);
        double totalQuantity = ambiguousList.fold(
            ...
            (sum, ingredient) =>
                sum +
                convertToBaseUnit(ingredient.quantity, ingredient.unit) /
                    _ambiguousConversionTable[normalizeUnit(largestUnit)]!);
        combinedQuantities[key] = totalQuantity; // Consolidated to largest unit
        finalUnits[key] = largestUnit;
      }
    }
    ...
    return combinedQuantities.entries.map((entry) {
      ...
      // Convert to the larger unit if it makes sense
      if (canConvertToLargerUnit(quantity, unit)) {
        String largerUnit = _largerUnits[unit]!;
        double conversionFactor = _volumeConversionTable.containsKey(unit)
            ? _volumeConversionTable[largerUnit]! /
                _volumeConversionTable[unit]!
            : _weightConversionTable[largerUnit]! /
                _weightConversionTable[unit]!;
